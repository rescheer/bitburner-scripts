...
export function readPortObject(handle) {
  if (Object.hasOwn(handle, 'empty') && !handle.empty()) {
 ...
 * @returns {(Object|Array|null)} Null if port is empty or invalid JSON
 */
 ...
 * @param {boolean} [overwrite=true] If false, function will fail if port contains data
 * @returns {boolean} True if successful
 */
export function tryWritePortObject(handle, objData, overwrite = true) {
  if (Object.hasOwn(handle, 'empty')) {
    if (!overwrite && Object.hasOwn(handle, 'empty') && !handle.empty()) {
...
/**
 * Pushes a new value into an array inside of a port. Fails if the port
 * contains an object that is not an array.
 * @param {NetscriptPort} handle - Handle of the port to be pushed to
 * @param {any} data - Data to push into the array
 * @returns True if successful
 */
export function pushPortArray(handle, data) {
  const portData = peekPortObject(handle);
  if (Object.hasOwn(handle, 'empty') && !handle.empty()) {
    if (Array.isArray(portData) && data) {
      portData.push(data);
      return tryWritePortObject(handle, portData);
    }
  } else {
    return tryWritePortObject(handle, [data]);
  }
  return false;
}
