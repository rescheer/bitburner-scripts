...
function processString(string, obj) {
  ...
  if (splitString.length === 2 && splitString[0] && splitString[1]) {
    if (Object.hasOwn(obj, splitString[0]) && Object.hasOwn(obj[splitString[0]], splitString[1])) {
      return splitString;
    }
  }
  return null;
...
export function getSetting(ns, keyString, objOrFilename) {
  const data = typeof objOrFilename === 'string' ? JSON.parse(ns.read(objOrFilename)) : objOrFilename;
